...
uint8_t identify() {
    ...
    uint8_t buff[256 * 2];
    port_words_in(ATA_PRIMARY_DATA, buff, 256);
 ...
 *     Transfer 256 16-bit values, a uint16_t at a time, into your buffer from I/O port 0x1F0. (In assembler, REP port_words_in works well for this.)
 ...
 * (As before, do not use REP port_words_out when writing.) And remember to do a Cache Flush after
